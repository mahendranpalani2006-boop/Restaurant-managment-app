...
<?php
	...
	if (isset($_POST['btnAdd'])) {
		...
		if (!empty($title) && !empty($message) && empty($error['image'])) {
            ...
            $image = get_random_string($string, 4)."-".date("Y-m-d").".".$extension;
            $upload = move_uploaded_file($_FILES['image']['tmp_name'], 'upload/notification/'.$image);        	
            $upload_image = $image;

			// insert new data to menu table
			$sql_query = "INSERT INTO tbl_notification (title, message, image, link) VALUES (?, ?, ?, ?)";
			...
			if($result) {
				redirect("notification.php", "Notification added successfully...");
		    } else {
